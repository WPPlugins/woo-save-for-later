...
class Woo_Save_For_Later_Admin {
    ...
    public function enqueue_styles() {
        ...
        if (isset($_GET['page']) && !empty($_GET['page']) && ($_GET['page'] == 'save_for_later' || $_GET['page'] == 'wsfl-add-new' || $_GET['page'] == 'wsfl-premium' || $_GET['page'] == 'wsfl-get-started' || $_GET['page'] == 'wsfl-information' || $_GET['page'] == 'wsfl-edit-fee')) {
    ...
    public function enqueue_scripts() {
        ...
        if (isset($_GET['page']) && !empty($_GET['page']) && ($_GET['page'] == 'save_for_later' ||  $_GET['page'] == 'wsfl-premium' || $_GET['page'] == 'wsfl-get-started' || $_GET['page'] == 'wsfl-information' || $_GET['page'] == 'wsfl-edit-fee')) {
    ...
    public function dot_store_menu_save_for_later_lite() {
        ...
        if (empty($GLOBALS['admin_page_hooks']['dots_store'])) {
            add_menu_page(
                'DotStore Plugins', __('DotStore Plugins'), 'manage_option', 'dots_store', array($this, 'dot_store_menu_page'), WSFL_PLUGIN_URL . 'admin/images/menu-icon.png', 25
            );
        }
        ...
        add_submenu_page('dots_store', 'WooCommerce Save For Later ', __('WooCommerce Save For Later '), 'manage_options', 'save_for_later', array($this, 'wsfl_fee_list_page'));
    }
    ...
    public function woo_save_for_later_admin_menu() {
       // add_menu_page(WSFL_PLUGIN_ADMIN_MENU_SLUG, __(WSFL_PLUGIN_ADMIN_MENU_NAME, WSFL_PLUGIN_SLUG), 'manage_options', WSFL_PLUGIN_ADMIN_MENU_TITLE, 'woo_save_for_later_admin_menu_custom');
        ...
        function woo_save_for_later_admin_menu_custom() {
            global $wpdb, $wp, $post, $woocommerce;
            ...
            if (!get_option('wsflfree_plugin_notice_shown')) {
                ?>
                <div id="wsfl_dialog">
            ...
            <div class="wsfl-main-container" id="wsfl_main_contain_ids">
                <fieldset class="wsfl-global">
                    <legend><div class="wsfl-legennd-header"><h2><?php echo __(WSFL_PLUGIN_MENU_PAGE_TITLE, WSFL_PLUGIN_SLUG); ?></h2></div></legend>
                    <div class="wsfl-field-contain">
                        <form id="wsfl_plugin_form_id" method="post" action="<?php echo get_admin_url(); ?>admin-post.php" enctype="multipart/form-data" novalidate="novalidate">
                            ...
                            <table class="form-table">
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="set_fonts">
                                            <input type="text" value="<?php echo ($getPluginLinkText) ? $getPluginLinkText : 'Save for Later'; ?>" name="wsfl_page_text_title" class="set_wsfl_text_title">	
                                        </td>
                                    </tr>
                                    <tr>
                            ...
                            <p class="submit">
                                <input type="submit" value="<?php echo __(WSFL_PLUGIN_SAVE_BUTTON_TITLE, WSFL_PLUGIN_SLUG); ?>" class="button button-primary" id="wsfl_submit_plugin" name="wsfl_submit_plugin">
                                <input type="reset" value="<?php echo __(WSFL_PLUGIN_RESET_BUTTON_TITLE, WSFL_PLUGIN_SLUG); ?>" class="button button-primary" id="wsfl_reset_plugin" name="wsfl_reset_plugin">
                            </p>
                        </form>
                    </div>
                </fieldset>
    ...
    public function woo_save_for_later_add_option() {
        ...
        $pluginPageText = ( $_POST['wsfl_page_text_title'] ) ? $_POST['wsfl_page_text_title'] : 'Save for Later';
        $pluginPageName = ( $_POST['wsfl_page_name'] ) ? $_POST['wsfl_page_name'] : 'Save for Later';
        $wsflOptionArr = array();
    ...
    public function remove_page_trash_link($actions, $post) {
        ...
        if ($current_screen->post_type == 'page') {
            ...
            if ($post->post_title == "Save For Later" || $post->post_name == "save-for-later") {
                unset($actions['edit']);
                unset($actions['view']);
                unset($actions['trash']);
    ...
    public function wp_add_plugin_userfn_wsflfree() {
        ...
        if (!empty($email_id)) {
            $request_response = wp_remote_post($request_url, array('method' => 'POST',
                ...
                'blocking' => true,
                'headers' => array(),
                'body' => array('user' => array('plugin_id' => '26', 'user_email' => $email_id, 'plugin_site' => $log_url, 'status' => 1, 'activation_date' => $cur_date)),
                'cookies' => array()));
            update_option('wsflfree_plugin_notice_shown', 'true');
        }
    }
    ...
    public function welcome_pages_screen_woocommerce_save_for_later() {
        add_dashboard_page(
                'WooCommerce-Save-For-Later Dashboard', 'WooCommerce Save For Later Dashboard', 'read', 'woocommerce-save-for-later', array(&$this, 'welcome_screen_content_woocommerce_save_for_later')
        );
    }
